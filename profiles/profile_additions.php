 ...
 */
	...
	$this->tpl->add_css("
		.uc_logo_Alliance { 
			background:url('".$this->server_path."games/wow243/profiles/factions/alliance-icon.png');
		}
		.uc_logo_Horde { 
			background:url('".$this->server_path."games/wow243/profiles/factions/horde-icon.png');
		}
		.uc_name { 
			text-align: left;
			color: white;
		...
		.raideventicon { background: url('".$this->server_path."games/wow243/profiles/events/raid-icons.jpg') no-repeat; width: 59px; height: 59px; padding: 0 !important;}
		...
		.healthpowerbar li {
			background: url('".$this->server_path."games/wow243/profiles/bars/health_power_bars.png') repeat-x scroll 0 0 transparent;
			border-radius: 3px 3px 3px 3px;
			display: inline-block;
			margin-right: 11px;
			padding-left: 11px;
	...
	if($this->config->get('game_importer_apikey') != '' && $this->config->get('servername') != '' && !isset($chardata['status'])){
		...
		$a_profilers	= array(
			1	=> array(
				'icon'	=> $this->server_path.'games/wow243/profiles/profilers/askmrrobot.png',
				'name'	=> 'AskMrRobot.com',
	...
	// the non armory charview
		...
		$a_professions = array(
			0	=> array(
				'icon'			=> $this->server_path."games/wow243/profiles/professions/".(($member['prof1_name']) ? $member['prof1_name'] : '0').".jpg",
				'name'			=> $a_lang_profession[$member['prof1_name']],
				'progressbar'	=> $this->jquery->progressbar('profession1', 0, array('completed' => $member['prof1_value'], 'total' => 375, 'text' => '%progress%'))
			),
			1	=> array(
				'icon'			=> $this->server_path."games/wow243/profiles/professions/".(($member['prof2_name']) ? $member['prof2_name'] : '0').".jpg",
				'name'			=> $a_lang_profession[$member['prof2_name']],
				'progressbar'	=> $this->jquery->progressbar('profession2', 0, array('completed' => $member['prof2_value'], 'total' => 375, 'text' => '%progress%'))
			)
		);
		foreach ($a_professions as $v_professions){
			$this->tpl->assign_block_vars('professions', array(
		...
		$this->tpl->assign_vars(array(
			...
			'POWER_BAR_NAME'		=> $this->game->glang('uc_bar_mana'),//($this->game->glang('uc_bar_'.$member['second_name'])) ? $this->game->glang('uc_bar_'.$member['second_name']) : $member['second_name'],
